...
def button_process(shared_state, bz):
    while True:
        if bz.blue_button.is_pressed:
            ...
            if shared_state["menu_state"] == MenuState.MAIN:
                ...
                bz.yellow_led()
                show_numpad_input("")
                shared_state["focus_timer_length"] = get_numpad_input(25) * 60
                bz.off_led()
            ...
            if shared_state["menu_state"] == MenuState.MAIN:
                ...
                bz.yellow_led()
                show_numpad_input("")
                shared_state["rest_timer_length"] = get_numpad_input(5) * 60
                bz.off_led()
                shared_state["menu_state"] = MenuState.MAIN
            elif shared_state["menu_state"] == MenuState.START_TIMER_SUBMENU:
                bz.green_led()
...
def timer_process(shared_state, bz):
    while True:
        if shared_state["menu_state"] == MenuState.MAIN:
            show_menu()
...
if __name__ == "__main__":
    ...
    with Manager() as manager:
        ...
        p1 = Process(target=button_process, args=(shared_state, bz,))
        p2 = Process(target=timer_process, args=(shared_state, bz,))
